...
| F6.0 | 운영 안정성 | 타임아웃/토큰표준화/지연초기화/캐싱 | 높음 | 구현완료 |
...
| F1.4.5 | 데이터 보강 | FDR에 Change 부재 시 생성 | 정상화된 시계열 | <1초 | 내부 계산 |
...
| F1.6.5 | 토큰 만료 표준 처리 | KIS 오류 문자열 통일 판별 | 자동 재발급 | <1초 | KIS API |
...
| (선택) ClickHouse | 관측 지표 저장 | Langfuse 이벤트 | 대량 적재 | 운영 정책 |
